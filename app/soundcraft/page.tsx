...
export default function VFXTriggers() {
  const { isPlaying, isUsed, progress, volume, playEffect, setEffectVolume } =
    useAudio(effects);
                ...
                {categoryEffects.map((effect) => (
                  <div key={effect.id} className="space-y-3">
                    <button
                      ...
                      className={`w-full aspect-square rounded-xl flex flex-col items-center justify-center space-y-2 relative overflow-hidden
                    ${
                      isPlaying[effect.id]
